 ...
 */
  $.fn.simpleColor = function(options) {
    ...
    function buildChooser(index) {
      ...
      displayBox.css($.extend(options.displayCSS, { 'background-color': 'green' }));
	  displayBox.css($.extend(options.displayCSS, { 'color': 'black' }));
      displayBox.text('Change Color');
        ...
        // Build the chooser.
          ...
          for (var i=0; i<options.colors.length; i++) {
            ...
            function(event) {
			  ...
			  // important
              event.data.displayBox.css('background-color', 'green');
              event.data.chooser.hide();

              // If 'displayColorCode' is turned on, display the currently selected color code as text inside the button.
              if (options.displayColorCode) {
				event.data.displayBox.css('color', 'white');
                event.data.displayBox.text('Change Color');
              }
